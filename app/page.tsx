...
export default function Page(){
  ...
  const lier = () =>{
    if(typeBanque==='traditionnel'){
      ...
      setGdbAccount(newGDB); setModal('Succes lie: '+nomBanque+' a '+newGDB)
    } else {
      if(!plateforme || !emailId){ alert('Remplissez Plateforme et Email'); return }
      const newGDB = gdbAccount || genererGDB()
      localStorage.setItem('gdb_account',newGDB)
      setGdbAccount(newGDB); setModal('Verification envoyee a '+emailId)
    }
  }
  ...
  return(
    <div style={{minHeight:'100vh',background:'#f1f5f9',padding:14,display:'flex',flexDirection:'column',gap:14,fontFamily:'system-ui',paddingBottom:90}}>
      <div style={{background:'#fff',border:'2px solid #facc15',borderRadius:20,padding:16,display:'flex',flexDirection:'column',alignItems:'center'}}>
        ...
        <div style={{fontWeight:800,fontSize:11,color:'#a16207',textAlign:'center'}}>GDB • WEB3.0 • GCV {GCV}$ • P2P MONDIAL</div>
      ...
      <div style={{background:'#fff',border:'1px solid #e2e8f0',borderRadius:20,padding:14}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10}}>
          {[{id:'transferP2P',label:'Transférer'},{id:'lierBanque',label:'Lier Banque'},{id:'convert',label:'Convertir'},{id:'paiements',label:'Paiements'}].map(f=>(
      ...
      {modal && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.6)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:50,padding:10}}>
          <div style={{background:'#fff',borderRadius:20,padding:16,width:'100%',maxWidth:420,maxHeight:'94vh',overflowY:'auto'}}>
            ...
            {modal==='transferP2P' && (
              <div>
                ...
                <div style={{display:'flex',background:'#f1f5f9',borderRadius:20,padding:4,marginTop:16}}>
                  <button onClick={()=>setP2pType('interne')} style={{flex:1,background:p2pType==='interne'?'#fff':'transparent',border:'none',borderRadius:16,padding:10,fontWeight:800,fontSize:13}}>P2P Interne</button>
                  <button onClick={()=>setP2pType('externe')} style={{flex:1,background:p2pType==='externe'?'#fff':'transparent',border:'none',borderRadius:16,padding:10,fontWeight:800,fontSize:13}}>P2P Externe</button>
                </div>

                {p2pType==='interne' ? (
                  <div style={{border:'1px solid #e2e8f0',borderRadius:16,padding:14,marginTop:14}}>
                    <div style={{fontWeight:900}}>Transfert Interne Gargoura</div>
                    <div style={{marginTop:12}}><div style={{fontWeight:700,fontSize:13}}>Destinataire</div><div style={{display:'flex',gap:8,marginTop:4}}><input value={destPhone} onChange={e=>setDestPhone(e.target.value)} placeholder="Numéro de téléphone ou ID" style={{flex:1,padding:12,borderRadius:12,border:'1px solid #cbd5e1'}}/><button style={{width:44,borderRadius:12,border:'1px solid #e2e8f0'}}>QR</button></div></div>
                    <div style={{marginTop:12}}><div style={{fontWeight:700,fontSize:13}}>Cryptomonnaie</div><select style={{width:'100%',marginTop:4,padding:12,borderRadius:12,border:'1px solid #cbd5e1'}}><option>π PiCoin (314,159.00 USD)</option><option>BTC</option><option>SOL</option><option>XRP</option><option>XLM</option><option>USDT</option><option>e-CFA</option></select></div>
                    <div style={{marginTop:12}}><div style={{fontWeight:700,fontSize:13}}>Montant</div><input value={amountPi} onChange={e=>setAmountPi(e.target.value)} placeholder="0.00" type="number" style={{width:'100%',marginTop:4,padding:12,borderRadius:12,border:'1px solid #cbd5e1'}}/><div style={{fontSize:11,color:'#16a34a',marginTop:4}}>{usd.toLocaleString()} USD GCV</div></div>
                    <div style={{marginTop:12}}><div style={{fontWeight:700,fontSize:13}}>Note (optionnel)</div><input value={note} onChange={e=>setNote(e.target.value)} placeholder="Ajouter une note" style={{width:'100%',marginTop:4,padding:12,borderRadius:12,border:'1px solid #cbd5e1'}}/></div>
                    <button onClick={()=>setModal('P2P Interne: '+amountPi+' pi a '+destPhone)} style={{width:'100%',marginTop:14,background:'#1e40af',color:'#fff',border:'none',borderRadius:12,padding:14,fontWeight:800}}>Envoyer</button>
                  </div>
                ) : (
                  <div style={{border:'1px solid #e2e8f0',borderRadius:16,padding:14,marginTop:14}}>
                    <div style={{fontWeight:900}}>Transfert Externe</div>
                    <div style={{fontSize:12,color:'#1e40af',fontWeight:700,marginTop:4}}>Conforme ISO 20022 • SWIFT • SEPA</div>
                    <div style={{marginTop:12}}><div style={{fontWeight:700,fontSize:13}}>Zone de Transfert</div><select value={zoneTransfert} onChange={e=>setZoneTransfert(e.target.value)} style={{width:'100%',marginTop:4,padding:12,borderRadius:12,border:'1px solid #cbd5e1'}}><option>CEMAC</option><option>UEMOA</option><option>DOLLAR</option><option>JORDANIE GOLF</option><option>MOYEN-ORIENT</option><option>INTERNATIONAL</option><option>EUROPE SEPA</option></select></div>
                    <div style={{marginTop:12}}><div style={{fontWeight:700,fontSize:13}}>Banque Partenaire</div><select value={banquePartenaire} onChange={e=>setBanquePartenaire(e.target.value)} style={{width:'100%',marginTop:4,padding:12,borderRadius:12,border:'1px solid #cbd5e1'}}><option value="">Choisir une banque</option><option>Ecobank CEMAC</option><option>UBA UEMOA</option><option>Bank of Jordan</option><option>Emirates NBD Dubai</option><option>QNB Qatar</option><option>BNP Paribas</option><option>CitiBank Dollar</option></select></div>
                    <div style={{marginTop:12}}><div style={{fontWeight:700,fontSize:13}}>Numéro de Compte</div><input value={numCompteExt} onChange={e=>setNumCompteExt(e.target.value)} placeholder="Entrer le numéro de compte" style={{width:'100%',marginTop:4,padding:12,borderRadius:12,border:'1px solid #cbd5e1'}}/></div>
                    <div style={{marginTop:12}}><div style={{fontWeight:700,fontSize:13}}>Type de Devise</div><div style={{display:'flex',background:'#f1f5f9',borderRadius:20,padding:4,marginTop:4}}><button onClick={()=>setTypeDevise('crypto')} style={{flex:1,background:typeDevise==='crypto'?'#fff':'transparent',border:'none',borderRadius:16,padding:8,fontWeight:700,fontSize:12}}>Cryptomonnaies</button><button onClick={()=>setTypeDevise('locale')} style={{flex:1,background:typeDevise==='locale'?'#fff':'transparent',border:'none',borderRadius:16,padding:8,fontWeight:700,fontSize:12}}>Devises Locales</button></div>{typeDevise==='crypto' ? <select style={{width:'100%',marginTop:8,padding:12,borderRadius:12,border:'1px solid #cbd5e1'}}><option>π PiCoin (314,159.00 USD)</option><option>BTC</option><option>SOL</option><option>XRP</option><option>XLM</option><option>USDT</option></select> : <select style={{width:'100%',marginTop:8,padding:12,borderRadius:12,border:'1px solid #cbd5e1'}}><option>XAF CEMAC</option><option>XOF UEMOA</option><option>USD Dollar</option><option>JOD Dinar</option><option>SAR Riyal</option><option>AED Dirham</option><option>e-CFA CBDC</option></select>}</div>
                    <div style={{marginTop:12}}><div style={{fontWeight:700,fontSize:13}}>Montant</div><input value={amountPi} onChange={e=>setAmountPi(e.target.value)} placeholder="0.00" type="number" style={{width:'100%',marginTop:4,padding:12,borderRadius:12,border:'1px solid #cbd5e1'}}/></div>
                    <div style={{background:'#dcfce7',borderRadius:12,padding:10,marginTop:12,fontSize:12}}><b>Transfert instantané via système interopérable • Zone {zoneTransfert}</b></div>
                    <button onClick={()=>setModal('P2P Externe '+zoneTransfert+' vers '+banquePartenaire+' - '+amountPi+' pi')} style={{width:'100%',marginTop:14,background:'#1e40af',color:'#fff',border:'none',borderRadius:12,padding:14,fontWeight:800}}>⚡ Envoyer Externe</button>
                  </div>
                )}
            ...
            {modal==='receive' && (<div style={{textAlign:'center'}}><div style={{fontWeight:900}}>QR Code {gdbAccount}</div><div style={{background:'#fff',padding:10,borderRadius:16,marginTop:12,border:'2px solid #facc15',display:'inline-block'}}><img src={`https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${gdbAccount}`} alt="QR" style={{width:220,height:220}} /></div><button onClick={()=>setModal(null)} style={{width:'100%',marginTop:12,background:'#e2e8f0',border:'none',borderRadius:12,padding:10}}>Fermer</button></div>)}

            {modal==='convert' && (<div><div style={{fontWeight:900}}>Convertisseur GCV {GCV}$</div><input value={amountPi} onChange={e=>setAmountPi(e.target.value)} type="number" placeholder="Montant pi" style={{width:'100%',marginTop:12,padding:12,borderRadius:12,border:'1px solid #cbd5e1'}}/><div style={{background:'#000',color:'#fff',borderRadius:12,padding:12,marginTop:12}}>{usd.toLocaleString()} USD</div><button onClick={()=>setModal(null)} style={{width:'100%',marginTop:12,background:'#1e40af',color:'#fff',border:'none',borderRadius:12,padding:10}}>Fermer</button></div>)}

            {modal==='paiements' && (<div><div style={{fontWeight:900}}>Paiements</div><button onClick={()=>setModal('transferP2P')} style={{width:'100%',marginTop:12,padding:12,borderRadius:12,border:'1px solid #e2e8f0',background:'#fff',textAlign:'left'}}><b>P2P Mondial</b><div style={{fontSize:11,color:'#64748b'}}>Interne + Externe CEMAC UEMOA DOLLAR GOLF</div></button><button onClick={()=>setModal(null)} style={{width:'100%',marginTop:8,background:'#e2e8f0',border:'none',borderRadius:12,padding:10}}>Fermer</button></div>)}
          </div>
        </div>
      )}
