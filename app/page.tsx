...
<div style={{background:'#fff',border:'1px solid #e2e8f0',borderRadius:16,padding:12}}><div style={{fontWeight:900}}>Fonctionnalités Courantes • 12</div><div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:8,marginTop:12}}>{funcs.map(function(f){return (<button key={f.id} onClick={function(){open(f.id)}} style={{border:f.id==='convertir'?'2px solid #16a34a':'1px solid #e2e8f0',background:f.id==='convertir'?'#f0fdf4':'#fff',borderRadius:12,padding:'8px 4px',display:'flex',flexDirection:'column',alignItems:'center',gap:4}}><div style={{fontSize:16}}>{f.i}</div><div style={{fontSize:7,fontWeight:700,textAlign:'center'}}>{f.l}</div></button>)})}</div></div>
</div>
...
<button onClick={function(){open('trading')}} style={{border:'none',background:a==='trading'?'#dbeafe':'none',fontSize:9,color:a==='trading'?'#1e40af':'#64748b',borderRadius:12,padding:'6px 5px',display:'flex',flexDirection:'column',alignItems:'center',minWidth:42}}><div style={{fontSize:18}}>📈</div>Trading</button>
...
{m!=='transferer' && m!=='virement' && m!=='pidex' && m!=='convertir' && m!=='receive' && (<div style={{background:'#fff',borderRadius:12,padding:12,marginTop:10}}><div style={{fontWeight:800}}>{m} • {gdb}</div><div style={{fontSize:11,color:'#64748b',marginTop:4}}>Module en construction - GDB unique {gdb} • PiCoin 314159 USD ref - Interopérable ISO20022</div></div>)}
