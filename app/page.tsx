  ...
  return(
    <div style={{minHeight:'100vh',background:'#f1f5f9',padding:14,display:'flex',flexDirection:'column',gap:14,fontFamily:'system-ui',paddingBottom:90}}>
      <div style={{background:'#fff',border:'2px solid #facc15',borderRadius:20,padding:16,display:'flex',flexDirection:'column',alignItems:'center'}}>
        ...
        <div style={{fontWeight:800,fontSize:11,color:'#a16207',letterSpacing:2,textAlign:'center'}}>GDB • WEB3.0 • PI NETWORK • GCV {GCV}$ • BANQUE UNIVERSELLE</div>
      ...
      <div style={{background:'#1e40af',border:'2px solid #facc15',borderRadius:18,padding:16,color:'#fff'}}>
        ...
        <div style={{fontSize:11,opacity:0.8,marginTop:4}}>Taux pivot GCV pour affichage. GDB agrège BTC, SOL, XRP, XLM, BNB, MATIC, USDT, USDC, CBDC.</div>
          ...
          <div style={{background:'#fff',borderRadius:20,padding:20,width:'100%',maxWidth:400,maxHeight:'85vh',overflowY:'auto'}}>
            ...
            {modal==='convert' && (
              <div>
                ...
                <div style={{background:'#000',color:'#facc15',borderRadius:12,padding:14,marginTop:12}}>
                  <div style={{fontSize:12}}>Valeur GCV Pivot</div>
                  <div style={{fontSize:20,fontWeight:900,color:'#fff'}}>{usd.toLocaleString()} USD</div>
                </div>
                <div style={{display:'flex',flexDirection:'column',gap:8,marginTop:14,maxHeight:300,overflowY:'auto'}}>
                  {[
                    {sym:'BTC', name:'Bitcoin', rate: usd / 65000},
                    {sym:'SOL', name:'Solana', rate: usd / 145},
                    {sym:'XRP', name:'Ripple', rate: usd / 0.62},
                    {sym:'XLM', name:'Stellar', rate: usd / 0.11},
                    {sym:'BNB', name:'BNB', rate: usd / 610},
                    {sym:'MATIC', name:'Polygon', rate: usd / 0.85},
                    {sym:'USDT', name:'Tether', rate: usd},
                    {sym:'USDC', name:'USDC', rate: usd},
                    {sym:'e-EUR', name:'Euro Numérique CBDC', rate: usd * 0.92},
                    {sym:'e-CNY', name:'Yuan Numérique CBDC', rate: usd * 7.2},
                    {sym:'e-CFA', name:'e-CFA BCEAO CBDC', rate: usd * 605},
                  ].map(c=>(
                    <div key={c.sym} style={{display:'flex',justifyContent:'space-between',background:'#f8fafc',border:'1px solid #e2e8f0',borderRadius:10,padding:'10px 12px'}}>
                      <div><div style={{fontWeight:800,fontSize:12}}>{c.sym}</div><div style={{fontSize:9,color:'#64748b'}}>{c.name}</div></div>
                      <div style={{fontWeight:800,fontSize:12}}>{c.rate.toLocaleString(undefined,{maximumFractionDigits:4})} {c.sym}</div>
                    </div>
                  ))}
                </div>
                <div style={{background:'#f0fdf4',border:'1px solid #bbf7d0',borderRadius:10,padding:10,marginTop:12,fontSize:10}}><b>Mission GDB:</b> Promouvoir π utilité réelle, mais agréger toutes monnaies électroniques et CBDC futures.</div>
                ...
                <button onClick={()=>setModal('paiements')} style={{width:'100%',marginTop:8,background:'#e2e8f0',border:'none',borderRadius:12,padding:10}}>← Retour Paiements</button>
              </div>
            )}

            {(modal==='mobile' || modal==='bills' || modal==='virement' || modal==='dex' || modal==='wallet' || modal==='trading') && (
              <div><div style={{fontWeight:900}}>{modal.toUpperCase()} • GDB {gdbAccount}</div><div style={{fontSize:12,color:'#64748b',marginTop:6}}>Service GCV opérationnel. 1 π = {GCV.toLocaleString()} USD. Compte {gdbAccount} prêt pour {modal}.</div>{modal==='dex' && (<div><input value={amountPi} onChange={e=>setAmountPi(e.target.value)} type="number" placeholder="π à trader" style={{width:'100%',marginTop:12,padding:12,borderRadius:12,border:'1px solid #cbd5e1'}}/><div style={{background:'#000',color:'#facc15',borderRadius:12,padding:12,marginTop:12,fontSize:12}}>Valeur: <b style={{color:'#fff'}}>{usd.toLocaleString()} USD</b></div></div>)}<button onClick={()=>setModal(null)} style={{width:'100%',marginTop:14,background:'#1e40af',color:'#fff',border:'none',borderRadius:12,padding:10,fontWeight:700}}>Fermer</button><button onClick={()=>setModal('paiements')} style={{width:'100%',marginTop:8,background:'#e2e8f0',border:'none',borderRadius:12,padding:10}}>← Retour Paiements</button></div>
            )}

            {!['paiements','transfer','receive','convert','mobile','bills','virement','dex','wallet','trading'].includes(modal) && (
              <div><div style={{fontWeight:900}}>{modal} • {gdbAccount}</div><div style={{fontSize:12,color:'#64748b',marginTop:6}}>Fonctionnalité GCV opérationnelle à {GCV.toLocaleString()} USD</div><button onClick={()=>setModal(null)} style={{width:'100%',marginTop:14,background:'#1e40af',color:'#fff',border:'none',borderRadius:12,padding:10}}>Fermer</button></div>
            )}
          </div>
        </div>
      )}
