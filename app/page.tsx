...
export default function Home(){
 ...
 const authPi=async()=>{
  ...
  try{
   ...
   const supabase=getSupabase()
   if(supabase) await supabase.from('users').upsert({pi_uid:auth.user.uid, username:auth.user.username})
  ...
  if(Pi && piUser && supabase){
   Pi.createPayment({amount:parseFloat(piAmt), memo:'GDB vers '+phone, metadata:{phone}},{
    ...
    onReadyForServerCompletion:async(id:string,tx:any)=>{
     ...
     alert('Paiement Pi OK! '+id);loadTxs()
    },
    onCancel:()=>alert('Annulé'),onError:(e:any)=>alert('Erreur '+e)
   })
  }else if(supabase){
   ...
   alert('Envoi '+piAmt+' Pi vers '+phone+' enregistré!');setPhone('');setPiAmt('');loadTxs()
  }else{
   alert('Supabase pas configuré sur Vercel - va dans Settings > Env Vars')
  }
 }
 ...
 return(
  <div style={{background:'#0f172a',minHeight:'100vh',color:'white',padding:'12px',paddingBottom:'80px'}}>
   <div style={{background:'#1e293b',padding:'12px',borderRadius:'12px',border:'2px solid #facc15',display:'flex',justifyContent:'space-between'}}>
    <b style={{color:'#facc15'}}>GDB V9.1 FIX</b>
   ...
   <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px',marginTop:'12px'}}>
    <div style={{background:'#facc15',color:'black',padding:'12px',borderRadius:'12px'}}><small>Solde Pi GCV</small><h2>1,250 pi</h2></div>
   ...
   <div style={{background:'#1e293b',padding:'14px',borderRadius:'12px',marginTop:'12px',border:'1px solid #facc15'}}>
    <b style={{color:'#facc15'}}>Envoyer Pi V9.1</b>
    ...
    <button onClick={doSend} style={{width:'100%',marginTop:'10px',padding:'14px',background:'#facc15',border:'none',borderRadius:'10px',fontWeight:'bold',color:'black'}}>{piUser?'Payer avec Pi SDK':'Envoyer (Supabase)'}</button>
   ...
   <div style={{marginTop:'12px'}}><b>Historique Supabase</b><div style={{display:'grid',gap:'6px',marginTop:'8px'}}>{txs.map((t,i)=><div key={i} style={{background:'#1e293b',padding:'10px',borderRadius:'8px',border:'1px solid #334155'}}><b>{t.type} {t.amount_pi} pi</b> <small style={{color:'#22c55e'}}>{t.mode}</small><br/><small style={{color:'#94a3b8'}}>{t.user_phone} - {t.amount_xaf} XAF</small></div>)}{txs.length==0&&<small style={{color:'#64748b'}}>Aucune transaction</small>}</div></div>
   {showKYC&&<div style={{position:'fixed',top:0,left:0,right:0,bottom:0,background:'rgba(0,0,0,0.9)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:50,padding:'14px'}}><div style={{background:'#1e293b',padding:'16px',borderRadius:'12px',border:'2px solid #facc15',width:'100%',maxWidth:'320px'}}><h3 style={{color:'#facc15',margin:0}}>KYC V9.1</h3><input placeholder="Nom complet" id="nom" style={{width:'100%',padding:'10px',marginTop:'8px',borderRadius:'8px',background:'#0f172a',border:'1px solid #334155',color:'white'}}/><input placeholder="Carte Nationale" id="carte" style={{width:'100%',padding:'10px',marginTop:'8px',borderRadius:'8px',background:'#0f172a',border:'1px solid #334155',color:'white'}}/><div style={{display:'flex',gap:'8px',marginTop:'10px'}}><button onClick={()=>setShowKYC(false)} style={{flex:1,padding:'10px',background:'#0f172a',color:'white',border:'1px solid #334155',borderRadius:'8px'}}>Annuler</button><button onClick={async()=>{const nom=(document.getElementById('nom') as HTMLInputElement).value;const carte=(document.getElementById('carte') as HTMLInputElement).value;const supabase=getSupabase();if(supabase) await supabase.from('kyc').insert({phone:phone||'66 78 75 46', nom, carte, status:'pending'});setKycOk(true);setShowKYC(false);alert('KYC envoyé!')}} style={{flex:1,padding:'10px',background:'#22c55e',border:'none',borderRadius:'8px',fontWeight:'bold'}}>Valider KYC</button></div></div></div>}
  </div>
 )
                                              }
