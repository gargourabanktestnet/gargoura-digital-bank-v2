...
<div style={{background:'#1e3a8a',borderRadius:20,padding:16,color:'#fff'}}><div style={{fontSize:13}}>Solde Total • {gdb}</div><div style={{fontWeight:900,fontSize:24,marginTop:8}}>1 pi = 314 159,00 USD</div><div style={{display:'flex',alignItems:'center',gap:6,marginTop:8}}><div style={{width:8,height:8,background:'#22c55e',borderRadius:8}}></div><div style={{fontSize:11,color:'#86efac',fontWeight:700}}>Le système est en ligne • AMM Actif</div></div><div style={{display:'flex',gap:8,marginTop:12}}><button onClick={function(){open('transferer')}} style={{flex:1,background:'#16a34a',color:'#fff',border:'none',borderRadius:20,padding:10,fontWeight:800}}>Envoyer</button><button onClick={function(){setM('receive')}} style={{flex:1,background:'#fff',color:'#1e3a8a',border:'none',borderRadius:20,padding:10,fontWeight:800}}>Recevoir</button></div></div>
<div style={{background:'#fff',border:'1px solid #e2e8f0',borderRadius:16,padding:12}}><div style={{fontWeight:900}}>Fonctionnalités Courantes • 12</div><div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:8,marginTop:12}}>{funcs.map(function(f){return (<button key={f.id} onClick={function(){open(f.id)}} style={{border:f.id==='pidex'?'2px solid #a855f7':'1px solid #e2e8f0',background:f.id==='pidex'?'#faf5ff':'#fff',borderRadius:12,padding:'8px 4px',display:'flex',flexDirection:'column',alignItems:'center',gap:4}}><div style={{fontSize:16}}>{f.i}</div><div style={{fontSize:7,fontWeight:700,textAlign:'center'}}>{f.l}</div></button>)})}</div></div>
</div>
...
{['accueil','transferer','pidex','services','innovation','securite','support'].map(function(s){return (<button key={s} onClick={function(){open(s)}} style={{border:'none',background:a===s?'#dbeafe':'none',fontSize:9,color:a===s?'#1e40af':'#64748b',borderRadius:12,padding:'6px 5px'}}>{s.slice(0,4)}</button>)})}
</div>
...
{m!=='transferer' && m!=='virement' && m!=='pidex' && m!=='receive' && (<div style={{background:'#fff',borderRadius:12,padding:12,marginTop:10}}><div style={{fontWeight:800}}>{m} • {gdb}</div><div style={{fontSize:11,color:'#64748b',marginTop:4}}>Module en construction - GDB unique {gdb} • PiCoin 314159 USD ref</div></div>)}
